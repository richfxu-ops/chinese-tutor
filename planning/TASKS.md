...
- [ ] (P3) Watch 14B serve latency (3.9s mean × up-to-3 calls/turn). The old escape hatch (7B for auxiliary calls) is gone — the 7B was deleted 2026-07-16, so reinstating it costs a Colab retrain → merge → quantize. Cheaper levers first: trim the multi-call serve layer #ship
...
- [x] Retrain on Colab A100 (2026-07-12): 14B QLoRA via the disk-managed notebook (sanity run → full train → eval → merge → GGUF, intermediates deleted as consumed) → ~9GB Q4_K_M into outputs/, 7B kept as hsk5-tutor-q4_k_m.7b-v2.gguf rollback (7B since deleted — DECISIONS 2026-07-16) #train #run
